<div class="container-fluid">	
			<div class="row">
				...
				<div class="col-xs-12 col-sm-3">
					...
					<a href="<?=base_url()?>admin/<?=$term?>/form_blog?action=create"  class="btn btn-primary "><i class="entypo-plus-circled"></i>Add New </a> 
				</div>
                    ...
                    <tbody>
						...
						<?php foreach($data as $row){ ?>
							<tr>
							   <td><?=$i?></td>
							   <td><?=$row->en_title?></td>
							   <td><i id="published-icon-<?php echo $row->id ?>" onclick="is_published(<?php echo $row->id ?>, '<?php echo base_url()?>admin/<?=$term?>/publish?id=<?php echo $row->id ?>')" class="glyphicon glyphicon-<?php if($row->is_published) echo "check";else echo "unchecked"; ?>"></i></td>
							   <td><i id="featured-icon-<?php echo $row->id ?>" onclick="is_featured(<?php echo $row->id ?>, '<?php echo base_url()?>admin/<?=$term?>/feature?id=<?php echo $row->id ?>')" class="glyphicon glyphicon-<?php if($row->is_featured) echo "check";else echo "unchecked"; ?>"></i></td>
								...
								<td class="text-center">
									<div class="btn-group">
											...
											<ul class="dropdown-menu dropdown-default pull-right" role="menu">
												<li>
													<a href="<?=base_url()?>admin/<?php echo $term; ?>/form_blog?action=update&id_blog=<?=$row->id?>">
