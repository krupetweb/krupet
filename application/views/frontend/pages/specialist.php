				...
				<div id="search-panel-container" style=" <?php if(isset($banner)){ if(!empty($banner)){ echo "background-image: url(".base_url().$banner[0]->image.")"; }} ?> ">    
							...
							<div id="search-panel" class="container" >
								<div class="row" style="padding:10px;">
									<div class="col-xs-12 ">
										<?php if(isset($_GET['specification'])){?>
											<h4 class="heading"><?php echo $_GET['specification'];?></h4>
										<?php }else{ ?>
											<h4 class="heading">Specialist</h4>
										<?php }?>
									</div>
...
<script>
 //Auto Completed
	...
	function search(){
		...
		key			=$("#key").val();
		key 		=key.replace(/ /gi, "+");
		window.location.replace("<?php echo base_url().$lang; ?>/specialist?province="+province+"&distrit="+distrit+"&specification="+specification+"&key="+key);
