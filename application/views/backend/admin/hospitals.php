			   ...
			   <div class="container-fluid">
					<div class="row">
						...
						<div class="col-xs-12">
								<a href="<?=base_url()?>admin/hospital_form/create/0"  class="btn btn-primary pull-right"><i class="entypo-plus-circled"></i>Add New </a> 
			   ...
			   <div class="container-fluid">
					...
					<div class="row">
						...
						<div class="col-xs-2">
							<br /><a href="#" onclick="searchx()"  class="btn btn-primary pull-right"><i class='fa fa-search'></i> Search</a> 
						</div>
					</div>
			   </div>
			   <hr />
			   <div class="col-xs=12">
						...
						<?php foreach($data as $row){ ?>
							<tr>
							   ...
							   <td>
									<?php
										if($row->is_published){
											echo '<i id="published_icon_'.$row->id.'" onclick="is_published('.$row->id.', \''.base_url().'admin/update_published_status/tbl_hospitals/'.$row->id.'\')" class="glyphicon glyphicon-check"></i>';
										}else{
											echo '<i id="published_icon_'.$row->id.'" onclick="is_published('.$row->id.', \''.base_url().'admin/update_published_status/tbl_hospitals/'.$row->id.'\')" class="glyphicon glyphicon-unchecked"></i>';
										}
									?>
							   </td>
							  <td>
									<i id="featured_icon_<?php echo $row->id?>" onclick="is_featured(<?php echo $row->id?>, '<?php echo base_url()?>admin/update_featured_status_of_hospital')" class="glyphicon glyphicon-<?php if($row->is_featured) echo 'check'; else echo 'unchecked'; ?>"></i>
								</td>
							   ...
							   <td>
									 <div class="btn-group">
											...
											<ul class="dropdown-menu dropdown-default pull-right" role="menu">
												<li>
													<a href="<?=base_url()?>admin/hospital_form/update/<?=$row->id?>">
												...
												<li class="divider"></li>
												<li>
													<a href="#" onclick="confirm_modal('<?=base_url()?>admin/hospital_cud/delete/<?=$row->id?>')">
				...
				<script type="text/javascript">
					...
					function get_distrits_province_id(id_province) {
						if(id_province!=0){
							$.ajax({
								url: "<?php echo base_url();?>admin/get_distrits_province_id/"+id_province+"?distrit=<?php echo isset($_GET['distrit'])?$_GET['distrit']:'';?>",
								success: function(response){
					...
					function get_types_category_id(id_category) {
						if(id_category!=0){
							$.ajax({
								url: "<?php echo base_url();?>admin/get_type_category_id/"+id_category+"?type=<?php echo isset($_GET['type'])?$_GET['type']:'';?>",
								success: function(response){
									$('#type').html(response);
								}
							});
